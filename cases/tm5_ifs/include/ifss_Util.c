...
int InitSaveData()
{
  fpDat = fopen("ifss.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : ifss.dat");
    exit(1);
  }
  return 0;
}
...
FILE *fpMatlab;
  ...
  fpMatlab = fopen("ifss.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : ifss.m");
    exit(1);
  }

  fprintf(fpMatlab, "load ifss.dat;\n");
  fprintf(fpMatlab, "%sc = ifss;\n", prefix);
  fprintf(fpMatlab, "clear ifss;\n");
