...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{   
   double Ynew[16], Fcn0[16], Fcn[16],
      dFdT[16],
      Jac0[102], Ghimj[102];
   double K[16*ros_S];   
   double H, T, Hnew, HC, HG, Fac, Tau; 
   double Err, Yerr[16];
   int Pivot[16], Direction, ioffset, j, istage;
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
  ...
  /*~~~>   Compute the stages  */
   for (istage = 1; istage <= ros_S; istage++) {
      ...
      /* Current istage offset. Current istage vector is K[ioffset:ioffset+16-1] */
      ioffset = 16*(istage-1);
	 
      /* For the 1st istage the function has been computed previously */
      if ( istage == 1 )
	   WCOPY(16,Fcn0,1,Fcn,1);
      else { /* istage>1 and a new function evaluation is needed at current istage */
	if ( ros_NewF[istage-1] ) {
	   WCOPY(16,Y,1,Ynew,1);
	   for (j = 1; j <= istage-1; j++)
	     WAXPY(16,ros_A[(istage-1)*(istage-2)/2+j-1],
                   &K[16*(j-1)],1,Ynew,1); 
  ...
  /*~~~>  Compute the new solution   */
   WCOPY(16,Y,1,Ynew,1);
   for (j=1; j<=ros_S; j++)
       WAXPY(16,ros_M[j-1],&K[16*(j-1)],1,Ynew,1);

  /*~~~>  Compute the error estimation   */
   WSCAL(16,ZERO,Yerr,1);
   for (j=1; j<=ros_S; j++)    
       WAXPY(16,ros_E[j-1],&K[16*(j-1)],1,Yerr,1);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/   
{
  /*~~~> Local variables */     
   ...
   WAXPY(16,(-ONE),Fcn0,1,dFdT,1);
   WSCAL(16,(ONE/Delta),dFdT,1);
...
{   
  ...
  /*~~~>    Construct Ghimj = 1/(H*ham) - Jac0 */
     WCOPY(102,Jac0,1,Ghimj,1);
     WSCAL(102,(-ONE),Ghimj,1);
...
{   
  ...
  /*~~~> Note: for a full matrix use Lapack:
      NRHS = 1
      DGETRS( 'N', 16 , NRHS, A, 16, Pivot, b, 16, INFO ) */
