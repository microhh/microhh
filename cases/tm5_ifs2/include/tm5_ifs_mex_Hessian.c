...
/* Check for the right number and size of input arguments */
 if ( nrhs != 3 ) {
   mexErrMsgTxt("tm5_ifs_Hessian requires 3 input vectors: V(16), F(3), RCT(46)");
 }
 mrows =  mxGetM(prhs[0]); mcols = mxGetN(prhs[0]);
 if ( ( mrows != 16 )||( mcols != 1 ) ) {
   mexPrintf("First tm5_ifs_Hessian input argument is of size V(%d,%d).",  
               mrows, mcols);
   mexErrMsgTxt("First tm5_ifs_Hessian input argument should be a column vector V(16,1)");
 }
 ...
 if ( (  mrows != 46 )||( mcols != 1 ) ) {
   mexPrintf("Third tm5_ifs_Hessian input argument is of size RCT(%d,%d).",  
               mrows, mcols);
   mexErrMsgTxt("Third tm5_ifs_Hessian input argument should be a column vector RCT(46,1)");
 }
 
/* Check for the right number of output arguments */
 if ( nlhs != 1 ) {
   mexErrMsgTxt("tm5_ifs_Hessian requires 1 output column vector: HESS(72)");
 }
 ...
 plhs[0] = mxCreateDoubleMatrix(72,1,mxREAL);
 HESS = mxGetPr(plhs[0]);
