...
dict_par = [{'default':{},  'advec': { 'advec':{'swadvec':0}}, 'diff': { 'diff':{'swdiff':0}}, 'thermo': { 'thermo':{'swthermo':'0'}}, 'basestate':{'thermo':{'swupdatebasestate':0}},'buffer':{'buffer':{'swbuffer':0}}}]
...
def run(executable='microhh', mode='cpu', casedir='.', experiment='local'):
    ...
    cases = [mht.Case('bomex', casedir=casedir, rundir=experiment,options=options )]
...
def run_small(executable='microhh', mode='cpu', casedir='.', experiment='local'):
    ...
    if mode == 'cpumpi':
        options.extend(opt_mpi)
    cases = [mht.Case('bomex', casedir=casedir, rundir='{}_small'.format(experiment),options=options )]
...
def run_restart(executable='microhh', mode='cpu', casedir='.', experiment='local'):
    ...
    if mode == 'cpumpi':
        options.extend(opt_mpi)
    base_case = mht.Case('bomex', casedir=casedir, rundir=experiment,options=options )
