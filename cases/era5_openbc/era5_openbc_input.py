...
vgrid = Vertical_grid_2nd(_g.z, _g.zsize)
...
nc_main.createDimension('z', vgrid.kmax)
nc_init = nc_main.createGroup('init')

add_variable('z',    ('z'), nc_main, vgrid.z)
...
stop_thl = (thl[-1] - thl[-2]) / (vgrid.z[-1] - vgrid.z[-2])
stop_qt  = (qt [-1] - qt [-2]) / (vgrid.z[-1] - vgrid.z[-2])
...
"""
bs1 = thermo.create_moist_basestate(
...
thermo.save_moist_basestate(bs1, 'test/thermo_basestate_era5.0000000')
