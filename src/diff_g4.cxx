...
int cdiff_g4::diffc(double * restrict at, double * restrict a, double * restrict dzi4, double * restrict dzhi4, double visc)
{
  ...
  const double c0 = -1460./576.;
  const double c1 =   783./576.;
  const double c2 =   -54./576.;
  const double c3 =     1./576.;
...
#pragma ivdep
    for(int i=grid->istart; i<grid->iend; i++)
    {
      ...
      at[ijk] += visc * (c3*a[ijk-ii3] + c2*a[ijk-ii2] + c1*a[ijk-ii1] + c0*a[ijk] + c1*a[ijk+ii1] + c2*a[ijk+ii2] + c3*a[ijk+ii3])*dxidxi;
      at[ijk] += visc * (c3*a[ijk-jj3] + c2*a[ijk-jj2] + c1*a[ijk-jj1] + c0*a[ijk] + c1*a[ijk+jj1] + c2*a[ijk+jj2] + c3*a[ijk+jj3])*dyidyi;
      at[ijk] += visc * grad4x(grad4xbiasbot(a[ijk-kk2], a[ijk-kk1], a[ijk    ], a[ijk+kk1]) * dzhi4[kstart-1],
                               grad4x       (a[ijk-kk2], a[ijk-kk1], a[ijk    ], a[ijk+kk1]) * dzhi4[kstart  ],
                               grad4x       (a[ijk-kk1], a[ijk    ], a[ijk+kk1], a[ijk+kk2]) * dzhi4[kstart+1],
                               grad4x       (a[ijk    ], a[ijk+kk1], a[ijk+kk2], a[ijk+kk3]) * dzhi4[kstart+2])*dzi4[kstart];
    }
...
#pragma ivdep
      for(int i=grid->istart; i<grid->iend; i++)
      {
        ...
        at[ijk] += visc * (c3*a[ijk-ii3] + c2*a[ijk-ii2] + c1*a[ijk-ii1] + c0*a[ijk] + c1*a[ijk+ii1] + c2*a[ijk+ii2] + c3*a[ijk+ii3])*dxidxi;
        at[ijk] += visc * (c3*a[ijk-jj3] + c2*a[ijk-jj2] + c1*a[ijk-jj1] + c0*a[ijk] + c1*a[ijk+jj1] + c2*a[ijk+jj2] + c3*a[ijk+jj3])*dyidyi;
        at[ijk] += visc * grad4x(grad4x(a[ijk-kk3], a[ijk-kk2], a[ijk-kk1], a[ijk    ]) * dzhi4[k-1],
                                 grad4x(a[ijk-kk2], a[ijk-kk1], a[ijk    ], a[ijk+kk1]) * dzhi4[k  ],
                                 grad4x(a[ijk-kk1], a[ijk    ], a[ijk+kk1], a[ijk+kk2]) * dzhi4[k+1],
                                 grad4x(a[ijk    ], a[ijk+kk1], a[ijk+kk2], a[ijk+kk3]) * dzhi4[k+2])*dzi4[k];
      }
...
#pragma ivdep
    for(int i=grid->istart; i<grid->iend; i++)
    {
      ...
      at[ijk] += visc * (c3*a[ijk-ii3] + c2*a[ijk-ii2] + c1*a[ijk-ii1] + c0*a[ijk] + c1*a[ijk+ii1] + c2*a[ijk+ii2] + c3*a[ijk+ii3])*dxidxi;
      at[ijk] += visc * (c3*a[ijk-jj3] + c2*a[ijk-jj2] + c1*a[ijk-jj1] + c0*a[ijk] + c1*a[ijk+jj1] + c2*a[ijk+jj2] + c3*a[ijk+jj3])*dyidyi;
      at[ijk] += visc * grad4x(grad4x       (a[ijk-kk3], a[ijk-kk2], a[ijk-kk1], a[ijk    ]) * dzhi4[kend-2],
                               grad4x       (a[ijk-kk2], a[ijk-kk1], a[ijk    ], a[ijk+kk1]) * dzhi4[kend-1],
                               grad4x       (a[ijk-kk1], a[ijk    ], a[ijk+kk1], a[ijk+kk2]) * dzhi4[kend  ],
                               grad4xbiastop(a[ijk-kk1], a[ijk    ], a[ijk+kk1], a[ijk+kk2]) * dzhi4[kend+1])*dzi4[kend-1];
    }
...
#pragma ivdep
    for(int i=grid->istart; i<grid->iend; i++)
    {
      ...
      at[ijk] += visc * (c3*a[ijk-ii3] + c2*a[ijk-ii2] + c1*a[ijk-ii1] + c0*a[ijk] + c1*a[ijk+ii1] + c2*a[ijk+ii2] + c3*a[ijk+ii3])*dxidxi;
      at[ijk] += visc * (c3*a[ijk-jj3] + c2*a[ijk-jj2] + c1*a[ijk-jj1] + c0*a[ijk] + c1*a[ijk+jj1] + c2*a[ijk+jj2] + c3*a[ijk+jj3])*dyidyi;
      at[ijk] += visc * grad4x(grad4xbiasbot(a[ijk-kk2], a[ijk-kk1], a[ijk    ], a[ijk+kk1]) * dzi4[kstart-1],
                               grad4x       (a[ijk-kk2], a[ijk-kk1], a[ijk    ], a[ijk+kk1]) * dzi4[kstart  ],
                               grad4x       (a[ijk-kk1], a[ijk    ], a[ijk+kk1], a[ijk+kk2]) * dzi4[kstart+1],
                               grad4x       (a[ijk    ], a[ijk+kk1], a[ijk+kk2], a[ijk+kk3]) * dzi4[kstart+2])*dzhi4[kstart+1];
    }
...
#pragma ivdep
      for(int i=grid->istart; i<grid->iend; i++)
      {
        ...
        at[ijk] += visc * (c3*a[ijk-ii3] + c2*a[ijk-ii2] + c1*a[ijk-ii1] + c0*a[ijk] + c1*a[ijk+ii1] + c2*a[ijk+ii2] + c3*a[ijk+ii3])*dxidxi;
        at[ijk] += visc * (c3*a[ijk-jj3] + c2*a[ijk-jj2] + c1*a[ijk-jj1] + c0*a[ijk] + c1*a[ijk+jj1] + c2*a[ijk+jj2] + c3*a[ijk+jj3])*dyidyi;
        at[ijk] += visc * grad4x(grad4x(a[ijk-kk3], a[ijk-kk2], a[ijk-kk1], a[ijk    ]) * dzi4[k-2],
                                 grad4x(a[ijk-kk2], a[ijk-kk1], a[ijk    ], a[ijk+kk1]) * dzi4[k-1],
                                 grad4x(a[ijk-kk1], a[ijk    ], a[ijk+kk1], a[ijk+kk2]) * dzi4[k  ],
                                 grad4x(a[ijk    ], a[ijk+kk1], a[ijk+kk2], a[ijk+kk3]) * dzi4[k+1])*dzhi4[k];
      }
...
#pragma ivdep
    for(int i=grid->istart; i<grid->iend; i++)
    {
      ...
      at[ijk] += visc * (c3*a[ijk-ii3] + c2*a[ijk-ii2] + c1*a[ijk-ii1] + c0*a[ijk] + c1*a[ijk+ii1] + c2*a[ijk+ii2] + c3*a[ijk+ii3])*dxidxi;
      at[ijk] += visc * (c3*a[ijk-jj3] + c2*a[ijk-jj2] + c1*a[ijk-jj1] + c0*a[ijk] + c1*a[ijk+jj1] + c2*a[ijk+jj2] + c3*a[ijk+jj3])*dyidyi;
      at[ijk] += visc * grad4x(grad4x       (a[ijk-kk3], a[ijk-kk2], a[ijk-kk1], a[ijk    ]) * dzi4[kend-3],
                               grad4x       (a[ijk-kk2], a[ijk-kk1], a[ijk    ], a[ijk+kk1]) * dzi4[kend-2],
                               grad4x       (a[ijk-kk1], a[ijk    ], a[ijk+kk1], a[ijk+kk2]) * dzi4[kend-1],
                               grad4xbiastop(a[ijk-kk1], a[ijk    ], a[ijk+kk1], a[ijk+kk2]) * dzi4[kend  ])*dzhi4[kend-1];
    }
