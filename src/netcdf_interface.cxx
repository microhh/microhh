...
{
    int nc_check_output = NC_NOERR;

    if (master.get_mpiid() == 0)
    {
        if (mode == Netcdf_mode::Create)
            nc_check_output = nc_create(name.c_str(), NC_NOCLOBBER | NC_NETCDF4, &ncid);
        else if (mode == Netcdf_mode::Write)
            nc_check_output = nc_open(name.c_str(), NC_WRITE | NC_NETCDF4, &ncid);
        else if (mode == Netcdf_mode::Read)
            nc_check_output = nc_open(name.c_str(), NC_NOWRITE | NC_NETCDF4, &ncid);
    }

    master.broadcast(&nc_check_output, 1);
    nc_check( nc_check_output );
    ...
    {
        if (mode == Netcdf_mode::Create)
            nc_check_output =  nc_enddef(root_ncid);
    }
