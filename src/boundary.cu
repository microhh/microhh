...
namespace Boundary_g
{
  ...
  __global__ void calcGhostCellsBotw_4th(double * __restrict__ w,
                                         const int icells, const int icellsp,
                                         const int jcells, const int kstart)
  {
    const int i = blockIdx.x*blockDim.x + threadIdx.x;
  ...
  __global__ void calcGhostCellsTopw_4th(double * __restrict__ w,
                                         const int icells, const int icellsp,
                                         const int jcells, const int kend)
  {
    const int i = blockIdx.x*blockDim.x + threadIdx.x;
...
void Boundary::exec()
{
  ...
  if(grid->swspatialorder == "2")
  {
    ...
    for(FieldMap::const_iterator it=fields->sp.begin(); it!=fields->sp.end(); ++it)
    {
      ...
      Boundary_g::calcGhostCellsTop_2nd<<<grid2dGPU, block2dGPU>>>(&it->second->data_g[offs], grid->dzh_g, sbc[it->first]->bctop, 
  ...
  else if(grid->swspatialorder == "4")
  {
    ...
    Boundary_g::calcGhostCellsTop_4th<<<grid2dGPU, block2dGPU>>>(&fields->u->data_g[offs], mbctop, 
                                                        &fields->u->datatop_g[offs], &fields->u->datagradtop_g[offs],
                                                        grid->z_g,
                                                        grid->icells, grid->icellsp,
                                                        grid->jcells, grid->kend);
    ...
    Boundary_g::calcGhostCellsBot_4th<<<grid2dGPU, block2dGPU>>>(&fields->v->data_g[offs], mbcbot, 
                                                                 &fields->v->databot_g[offs], &fields->v->datagradbot_g[offs],
                                                                 grid->z_g,
                                                                 grid->icells, grid->icellsp,
                                                                 grid->jcells, grid->kstart);
    ...
    Boundary_g::calcGhostCellsTop_4th<<<grid2dGPU, block2dGPU>>>(&fields->v->data_g[offs], mbctop, 
                                                        &fields->v->datatop_g[offs], &fields->v->datagradtop_g[offs],
                                                        grid->z_g,
                                                        grid->icells, grid->icellsp,
                                                        grid->jcells, grid->kend);
    ...
    Boundary_g::calcGhostCellsBotw_4th<<<grid2dGPU, block2dGPU>>>(&fields->w->data_g[offs],
                                                                  grid->icells, grid->icellsp,
                                                                  grid->jcells, grid->kstart);
    cudaCheckError(); 

    Boundary_g::calcGhostCellsTopw_4th<<<grid2dGPU, block2dGPU>>>(&fields->w->data_g[offs],
                                                                  grid->icells, grid->icellsp,
                                                                  grid->jcells, grid->kend);
    cudaCheckError(); 
    ...
    for(FieldMap::const_iterator it=fields->sp.begin(); it!=fields->sp.end(); ++it)
    {
      ...
      Boundary_g::calcGhostCellsTop_4th<<<grid2dGPU, block2dGPU>>>(&it->second->data_g[offs], sbc[it->first]->bctop, 
                                                                   &it->second->datatop_g[offs], &it->second->datagradtop_g[offs],
                                                                   grid->z_g,
                                                                   grid->icells, grid->icellsp,
                                                                   grid->jcells, grid->kend);
