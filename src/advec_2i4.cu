...
namespace
{
                   ...
                   const int icells, const int jcells, const int kstart, const int kend, const int ijcells)
    {
        ...
        if (i < icells && j < jcells && k < kstart)
        {
            const int ijk = i + j*icells + k*ijcells;
            st[ijk] = 0;
        }
        else if (i < icells && j < jcells && k < kstart + 1)
        {
            const int ijk = i + j*icells + k*ijcells;
            st[ijk] = interp2(w[ijk-1], w[ijk]) * interp2(s[ijk-ijcells], s[ijk]);
        }
        else if (i < icells && j < jcells && k < kend - 1)
        {
            const int ijk = i + j*icells + k*ijcells;
            st[ijk] =  interp2(w[ijk-1], w[ijk]) * interp2(s[ijk-ijcells], s[ijk   ]);
        }
        ...
        {
            const int ijk = i + j*icells + k*ijcells;
            st[ijk] = interp2(w[ijk-1], w[ijk]) * interp4c(s[ijk-2*ijcells], s[ijk-ijcells], s[ijk], s[ijk+ijcells]);
        }
        else if (i < icells && j < jcells && k < kend)
        {
            const int ijk = i + j*icells + k*ijcells;
            st[ijk] =  interp2(w[ijk-1], w[ijk]) * interp2(s[ijk-ijcells], s[ijk   ]);
        }
                   ...
                   const int icells, const int jcells, const int kstart, const int kend, const int ijcells)
    {
        ...
        {
            const int ijk = i + j*icells + k*ijcells;
            st[ijk] = interp2(w[ijk-icells], w[ijk]) * interp4c(s[ijk-2*ijcells], s[ijk-ijcells], s[ijk], s[ijk+ijcells]);
        }
        else if (i < icells && j < jcells && k < kend)
        {
            const int ijk = i + j*icells + k*ijcells;
                   ...
                   const int icells, const int jcells, const int kstart, const int kend, const int ijcells)
    {
        ...
        {
            const int ijk = i + j*icells + k*ijcells;
            st[ijk] = w[ijk] * interp4c(s[ijk-2*ijcells], s[ijk-ijcells], s[ijk], s[ijk+ijcells]);
        }
        else if (i < icells && j < jcells && k < kend)
        {
            const int ijk = i + j*icells + k*ijcells;
        ...
        Field3d<TF>& advec_flux, const Field3d<TF>& fld)
{
    ...
    else if (fld.loc == gd.sloc)
    {
        ...
        throw std::runtime_error("Advec_2i4 cannot deliver flux field at that location");
}
#endif
