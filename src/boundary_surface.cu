...
__constant__ float zL_const[5000]; 
__constant__ float f_const[5000]; 
...
namespace BoundarySurface_g
{
  ...
  __device__ double findObuk(int &n, const double Ri, const double zsl)
  {
    ...
    if ( (f_const[n]-Ri) > 0 )
      while ( (f_const[n-1]-Ri) > 0 && n > 0) { --n; }
    else
      while ( (f_const[n]-Ri) < 0 && n < (nzL-1) ) { ++n; }

    const double zL0 = (n == 0 || n == nzL-1) ? zL_const[n] : zL_const[n-1] + (Ri-f_const[n-1]) / (f_const[n]-f_const[n-1]) * (zL_const[n]-zL_const[n-1]);
...
void BoundarySurface::prepareDevice()
{
  ...
  cudaMemcpyToSymbol(zL_const, zL_sl, nzL*sizeof(float));
  cudaMemcpyToSymbol(f_const,  f_sl,  nzL*sizeof(float));
