...
Advec_2i52<TF>::Advec_2i52(Master& masterin, Grid<TF>& gridin, Fields<TF>& fieldsin, Input& inputin) :
        ...
        for (int k=kstart; k<kend; ++k)
            for (int j=jstart; j<jend; ++j)
                ...
                for (int i=istart; i<iend; ++i)
                {
                    ...
                    cfl = std::max(cfl,
                            std::abs(interp6_ws(u[ijk-ii2], u[ijk-ii1], u[ijk], u[ijk+ii1], u[ijk+ii2], u[ijk+ii3]))*dxi
            ...
            for (int j=jstart; j<jend; ++j)
                ...
                for (int i=istart; i<iend; ++i)
                {
                    ...
                    ut[ijk] +=
                             ...
                             // w*du/dz
                             - ( rhorefh[k+1] * interp2(w[ijk-ii1+kk], w[ijk+kk]) * interp2(u[ijk   ], u[ijk+kk])
                               - rhorefh[k  ] * interp2(w[ijk-ii1   ], w[ijk   ]) * interp2(u[ijk-kk], u[ijk   ]) ) / rhoref[k] * dzi[k];
                }
    }
            ...
            for (int j=jstart; j<jend; ++j)
                ...
                for (int i=istart; i<iend; ++i)
                {
                    ...
                    vt[ijk] +=
                             ...
                             // w*dv/dz
                             - ( rhorefh[k+1] * interp2(w[ijk-jj1+kk], w[ijk+kk]) * interp2(v[ijk   ], v[ijk+kk])
                               - rhorefh[k  ] * interp2(w[ijk-jj1   ], w[ijk   ]) * interp2(v[ijk-kk], v[ijk   ]) ) / rhoref[k] * dzi[k];
                }
    }
...
template<typename TF>
void Advec_2i52<TF>::create(Stats<TF>& stats)
{
...
#ifndef USECUDA
template<typename TF>
double Advec_2i52<TF>::get_cfl(double dt)
{
...
template<typename TF>
unsigned long Advec_2i52<TF>::get_time_limit(unsigned long idt, double dt)
{
...
template<typename TF>
void Advec_2i52<TF>::exec(Stats<TF>& stats)
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Advec_2i52<TF>::get_advec_flux(Field3d<TF>& advec_flux, const Field3d<TF>& fld)
{
    auto& gd = grid.get_grid_data();
    ...
    else if (fld.loc == gd.sloc)
    {
        ...
        throw std::runtime_error("Advec_2i52 cannot deliver flux field at that location");
}

template class Advec_2i52<double>;
template class Advec_2i52<float>;
