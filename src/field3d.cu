...
void Field3d<TF>::calc_mean_profile(TF* tmp)
{
    ...
    reduce_interior<TF>(&fld_g[gd.memoffset], tmp, gd.itot, gd.istart, gd.iend, gd.jtot, gd.jstart, gd.jend, gd.kcells, 0, gd.icellsp, gd.ijcellsp, Sum_type);
    // Reduce jtot*kcells to kcells values
    reduce_all<TF>     (tmp, fld_mean_g, gd.jtot*gd.kcells, gd.kcells, gd.jtot, Sum_type, scalefac);
}
...
TF Field3d<TF>::calc_mean(TF* tmp)
{
    ...
    reduce_interior<TF>(&fld_g[gd.memoffset], tmp, gd.itot, gd.istart, gd.iend, gd.jtot, gd.jstart, gd.jend, gd.ktot, gd.kstart, gd.icellsp, gd.ijcellsp, Sum_type);
    // Reduce jtot*ktot to ktot values
    reduce_all<TF>     (tmp, &tmp[gd.jtot*gd.ktot], gd.jtot*gd.ktot, gd.ktot, gd.jtot, Sum_type, 1.);
    // Reduce ktot values to a single value
    reduce_all<TF>     (&tmp[gd.jtot*gd.ktot], tmp, gd.ktot, 1, gd.ktot, Sum_type, scalefac);
    // Copy back result from GPU
    cuda_safe_call(cudaMemcpy(&mean_value, &tmp[0], sizeof(TF), cudaMemcpyDeviceToHost));

    return mean_value;
}

template<typename TF>
TF Field3d<TF>::calc_max(TF* tmp)
{
    using namespace Tools_g;
    ...
    // Reduce 3D field excluding ghost cells and padding to jtot*ktot values
    reduce_interior<TF>(&fld_g[gd.memoffset], tmp, gd.itot, gd.istart, gd.iend, gd.jtot, gd.jstart, gd.jend, gd.ktot, gd.kstart, gd.icellsp, gd.ijcellsp, Max_type);
    // Reduce jtot*ktot to ktot values
    reduce_all<TF>     (tmp, &tmp[gd.jtot*gd.ktot], gd.jtot*gd.ktot, gd.ktot, gd.jtot, Max_type, scalefac);
    // Reduce ktot values to a single value
    reduce_all<TF>     (&tmp[gd.jtot*gd.ktot], tmp, gd.ktot, 1, gd.ktot, Max_type, scalefac);
    // Copy back result from GPU
    cuda_safe_call(cudaMemcpy(&max_value, &tmp[0], sizeof(TF), cudaMemcpyDeviceToHost));
