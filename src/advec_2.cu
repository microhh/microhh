...
//    const int ijs    = threadIdx.x + igc + (blockDim.x + 2 * igc) * (threadIdx.y + jgc); // Location ijk in __shared__ array
//    const int bdimxp = blockDim.x + 2 * igc; // Block size X with padding
//    const int bdimyp = blockDim.y + 2 * jgc; // Block size X with padding
...
//
//    // More cells than threads, so some threads have to read twice (or more..)
//    int ijsp, i2, j2, ijk2;
//    for (int n=0; n<nloads; ++n)
//    {
//      ijsp     = threadIdx.x + threadIdx.y * blockDim.y + n * blockDim.x * blockDim.y;
//      if(ijsp < bdimxp * bdimyp)
//      { 
//        i2       = ijsp % bdimxp + blockIdx.x * blockDim.x; 
//        j2       = floor((double)ijsp / bdimxp) + blockIdx.y * blockDim.y;
//        ijk2     = i2 + j2*jj + k*kk;
//        us[ijsp] = u[ijk2];
//        vs[ijsp] = v[ijk2];
//        ws[ijsp] = w[ijk2];
...
int cadvec_2::exec()
{
  ...
  //const int nloads = nsmem / (blocki * blockj) + ((nsmem % (blocki * blockj)) > 0);
