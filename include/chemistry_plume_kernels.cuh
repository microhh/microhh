...
#ifndef CHEMISTRY_PLUME_KERNELS_CUH
#define CHEMISTRY_PLUME_KERNELS_CUH

#include "chemistry_plume_kernels.h"
#include "chemistry.h"
#include "constants.h"

// The rate constant functions can be called from both CPU and GPU. Reuse them here:
namespace cpk = Chemistry_plume_kernels;

namespace Chemistry_plume_kernels_g
{
            ...
            const int kstride)
    {
        ...
        if (i < iend && j < jend && k < kend)
        {
            ...
            const TF c_h2o = max(qprof[k] * Constants::xmair<TF> * c_m * Constants::xmh2o_i<TF>, TF(1));

            const TF rconst0  = cpk::arr3(TF(1.7E-12), TF(-940), temp);
            const TF rconst1  = cpk::arr3(TF(1.E-14), TF(-490), temp);
            const TF rconst2  = cpk::arr3(TF(4.8E-11), TF(250), temp);
            const TF rconst3  = cpk::epr(TF(3.E-13), TF(460), TF(2.1E-33), TF(920), TF(1.4E-21), TF(2200), c_m, c_h2o, temp);
            const TF rconst4  = cpk::arr3(TF(2.9E-12), TF(-160), temp);
            const TF rconst5  = cpk::arr3(TF(2.8E-12), TF(-1800), temp) * c_h2;
            const TF rconst6  = cpk::arr3(TF(3.E-12), TF(-1500), temp);
            const TF rconst7  = cpk::arr3(TF(1.4E-13), TF(-2470), temp);
            const TF rconst8  = cpk::arr3(TF(1.8E-11), TF(110), temp);
            const TF rconst9  = cpk::troe_ifs(TF(3.6E-30), TF(4.1), TF(1.9E-12), TF(-0.2), TF(10), c_m, temp);
            const TF rconst10 = cpk::troe_ifs2(TF(1.3E-3), TF(-3.5), TF(9.7E14), TF(0.1), TF(10), c_m, TF(-11000), TF(-11080), temp);
            const TF rconst11 = cpk::arr3(TF(3.3E-12), TF(270), temp);
            const TF rconst12 = cpk::troe_no2oh(TF(3.2E-30), TF(4.5), TF(3.E-11), TF(10), c_m, temp);
            const TF rconst14 = cpk::rk28(TF(2.4E-14), TF(460), TF(6.51E-34), TF(1335), TF(2.69E-17), TF(2199), c_m, temp);
            const TF rconst16 = cpk::arr3(TF(2.45E-12), TF(-1775), temp);
            const TF rconst17 = cpk::arr3(TF(3.8E-13), TF(780), temp) * (TF(1)-(TF(1)/(TF(1) + cpk::arr3(TF(498), TF(-1160), temp))));
            const TF rconst18 = cpk::arr3(TF(3.8E-13), TF(780), temp) * (TF(1)/(TF(1.) + cpk::arr3(TF(498.), TF(-1160), temp)));
            const TF rconst19 = cpk::arr3(TF(2.8E-12), TF(300), temp);
            const TF rconst21 = cpk::arr3(TF(3.8E-12), TF(200), temp);
            const TF rconst22 = cpk::arr3(TF(5.5E-12), TF(125), temp);
            const TF rconst24 = cpk::troe_cooh(TF(5.9E-33), TF(1.4), TF(1.1E-12), TF(-1.3), TF(1.5E-13), TF(-0.6), TF(2.1E9), TF(-6.1), TF(0.6), c_m, temp);
            const TF rconst25 = cpk::arr3(TF(9.5E-14), TF(390), temp);
            const TF rconst26 = cpk::arr3(TF(5.5E-15), TF(-1880), temp);
            const TF rconst27 = cpk::k3rd_iupac(TF(8.6E-27), TF(3.5), TF(3.E-11), TF(1), TF(0.6), c_m, TF(0.5), temp);
            const TF rconst28 = cpk::arr3(TF(4.6E-13), TF(-1155), temp);
            const TF rconst29 = cpk::usr_o3_hv_h2o(temp, c_m, c_h2o, jval[Jval::o31d]);
            ...
            const TF var_no2  = max((no2[ijk]  + tno2[ijk]  * sdt) * cfactor, TF(0));
...
#endif // CHEMISTRY_PLUME_KERNELS_CUH
