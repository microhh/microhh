...
int InitSaveData()
{
  fpDat = fopen("mhh.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : mhh.dat");
    exit(1);
  }
  return 0;
}
...
FILE *fpMatlab;
  ...
  fpMatlab = fopen("mhh.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : mhh.m");
    exit(1);
  }

  fprintf(fpMatlab, "load mhh.dat;\n");
  fprintf(fpMatlab, "%sc = mhh;\n", prefix);
  fprintf(fpMatlab, "clear mhh;\n");
