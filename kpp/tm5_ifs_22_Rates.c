...
double TROE_kmeno2(double kzero, double mzero, double kinf, 
       	    double minf, double fmulti, double MN2, double TEMP)
       {
       double k0T, kinfT, kmeno2;
       k0T  = (kzero * pow((300./TEMP), mzero)) * MN2;
       kinfT = kinf * pow((300./TEMP), minf);
       kmeno2 = k0T / (1. + k0T / kinfT) * pow(fmulti, 
       	     1. / (1. + pow(log10(k0T/kinfT), 2)));
       return kmeno2;
       }

double TROE_hno4(double kzero, double mzero, double kinf, 
       	    double minf, double fmulti, double MN2, 
	    double TEMP)
       {
       double k0T, kinfT, znn;
       k0T  = (kzero * exp(mzero/TEMP)) * MN2;
       kinfT = kinf * exp(minf/TEMP);
       znn = 0.75 - (1.27 * log10(0.41));
       return (k0T * kinfT) / (k0T + kinfT) * pow(fmulti, 
       	      (log10(0.4) / (1. + pow((log10(k0T/kinfT))/znn, 2))));
       }
...
double kho2o3(double A, double b, double C, double TEMP)
       {
       return A * pow((TEMP/300.), b) * exp(C/TEMP);
       }  

double EPR_ev(double A1, double C1, double A2, double C2, 
           double mmult, double TEMP) 
      {               
      double K1, K2, EPR_p;
...
double TROE_ev_no2oh(double kzero, double mzero, double kinf, 
       	    double fmulti, double MN2, double TEMP)
       {
       double k0T, kinfT;
       k0T  = (kzero * pow((TEMP/300.), mzero)) * MN2;
       kinfT = kinf;
       return (k0T * kinfT * 
       	      pow(fmulti, (log10(0.4) / (1. + pow((log10(k0T/kinfT)), 2))))) /
	      (k0T + kinfT);
       }

double kohch4(double A, double b, double C, double TEMP)
       {
       return A * pow(TEMP, b) * exp(C/TEMP);
       }

double TROE_no2no3(double kzero, double mzero, double kinf, 
       	    double minf, double fmulti, double MN2, double TEMP)
       {
       double k0T, kinfT;
       k0T  = (kzero * pow((TEMP/300.), mzero)) * MN2;
       ...
       return (k0T * kinfT * pow(fmulti, 
       	      log10(0.35) / (1. + pow(log10(k0T/kinfT), 2)))) / (k0T + kinfT);
       }

double TROE_n2o5(double kzero, double mzero, double kinf, 
       	    double minf, double fmulti, double MN2, double c1, 
	    double c2, double TEMP)
       {
       double k0T, kinfT;
...
//double C_H2O = 5.344E+17;  //2.55E+17; // 1 percent, 0.01*Mval 
// double zo2 = 5.34E+18; // zo2 = 0.209476 * Mval
//double zno3 = 2.55E+7; // 1.E-3 ppb * 2.55E10
//double zn2o5 = 2.55E+7;

//Depositions: values are in cm/s and we assume a 1000 m high box...making the uC_M,0.5,TEMP)
...
/* End INLINED RCONST                                               */
  ...
  RCONST[1] = (kho2o3(2.03E-16,4.57,693.,TEMP));
  RCONST[2] = (ARR3(4.8E-11,250.,TEMP));
  RCONST[3] = (EPR_ev(2.2E-13,600.,1.9E-33,980.,C_M,TEMP));
  RCONST[4] = (ARR3(2.9E-12,-160.,TEMP));
  RCONST[5] = (ARR3(2.8E-12,-1800.,TEMP)*C_H2);
  RCONST[6] = (ARR3(1.4E-12,-1310.,TEMP));
  RCONST[7] = (ARR3(1.4E-13,-2470.,TEMP));
  RCONST[8] = (ARR3(1.8E-11,110.,TEMP));
  RCONST[9] = (TROE_no2no3(3.6E-30,-4.1,1.9E-12,0.2,10.,C_M,TEMP));
  RCONST[10] = (TROE_n2o5(1.E-3,-3.5,9.7E14,0.1,10.,C_M,-11000.,-11080.,
              TEMP));
  RCONST[11] = (ARR3(3.6E-12,270.,TEMP));
  RCONST[12] = (TROE_ev_no2oh(3.3E-30,-3.,4.1E-11,10.,C_M,TEMP));
  RCONST[14] = (RK28(2.4E-14,460.,6.51E-34,1335.,2.69E-17,2199.,C_M,
              TEMP));
  RCONST[16] = (kohch4(9.65E-20,2.58,-1082.,TEMP));
  RCONST[17] = (ARR3(3.8E-13,780.,TEMP));
  RCONST[18] = (ARR3(3.8E-13,780.,TEMP));
  RCONST[19] = (ARR3(1.82E-13,416.,TEMP));
  RCONST[21] = (ARR3(3.8E-12,200.,TEMP));
  RCONST[22] = (ARR3(5.5E-12,125.,TEMP));
  RCONST[24] = (TROE_cooh(5.9E-33,1.4,1.1E-12,-1.3,1.5E-13,-0.6,2.1E9,
              -6.1,0.6,Mval,TEMP));
  RCONST[25] = (ARR3(9.5E-14,390.,TEMP));
  RCONST[26] = (ARR3(5.5E-15,-1880.0,TEMP));
  RCONST[27] = (k3rd_iupac(8.6E-27,3.5,3.E-11,1.,0.6,C_M,0.5,TEMP));
  RCONST[28] = (ARR3(4.6E-13,-1155.,TEMP));
  RCONST[29] = (2.7E-06*SUN);
  RCONST[30] = (8.9E-03*SUN);
  RCONST[31] = (5.151E-5*SUN);
  RCONST[32] = ((2.620E-02+2.075E-01)*SUN);
  RCONST[33] = (5.531E-06*SUN);
  RCONST[34] = (3.734E-05*SUN);
  RCONST[35] = (2.845E-05*SUN);
  RCONST[36] = (8.881E-06*SUN);
  RCONST[37] = (eno);
  RCONST[38] = (erh);
  RCONST[39] = (vdo3);
  RCONST[40] = (vdno);
